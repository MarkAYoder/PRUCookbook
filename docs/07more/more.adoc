...
==== Discission
The code is mostly the same. `cycle` and `stall` end up in registers which
we can read it using prudebug.
...
|15  |Declaring cycle and stall.  The compiler with optimize these and just
keep them in registers.  We'll have to look at the cycle.lst file to see where
they are stored.
|20  |Enables `CYCLE`.
|25  |Reset `CYCLE`. It ignores the value assigned to it and always sets it
to 0.  cycle is on the right hand side to make the compiler give it it's own
register.
|27, 28|Reads the `CYCLE` and `STALL` values into registers.
|===
...
[subs=+quotes]
----
include::code/cycle.lst[lines=112..118]
----
...
[subs=+quotes]
----
include::code/cycle.lst[lines=145..151]
----
...
lines 27 and 28 you'll see `cycle` is 7 and `stall` is 2. `cycle` now
...
==== Solution
The `__xout()` and `__xin()` intrinsics are able to transfer up to 30 registers between
PRU 0 and PRU 1 in one cycle.  <<more_xout>> shows how `xout()` runnin on PRU 0 
...
PRU 1 waits at line 41 until PRU 0 signals it.  <<more_xin>> sends sneds and
...
==== Discussion
<<more_zout_lines>> shows the line-by-line for `xout.c`
...
|3    |A different resouce so PRU 0 can receive a signal from PRU 1.
|10-19|`dmemBug` holds the data to be sent to PRU1.  Each will be transfered
to its corresponding register by `xout()`
|22-23|Define the interupts we're using.
|28-29|Clear the interrupts.
|32-37|Initialize dmemBuf with easy to recognize values.
|41   |Wait for PRU 1 to signal.
|46   |`__xout()` does a direct transfer to PRU 1. 
Page 92 of 
http://www.ti.com/lit/ug/spruhv7b/spruhv7b.pdf[PRU Optimizing C/C++ Compiler, v2.2, User's Guide]
shows how to use `xout()`.  14 says to do a direct transfer to PRU 1.  If the
first argument is 10, 11 or 12, the data is transfered to one of three 
scratchpad memories that PRU 1 can access later.  5 says to start transfering
with register `r5` and use as many regsiters as need to transfer all of
`dmemBuf`.
|49   |Clear the interupt.
|===

<<more_xin_lines>> shows the line-by-line for `xin.c`.

[[more_xin_lines]]
.xout.c Line-by-line[cols="1,9"]
|===
|Line |Explanation

|9-16 |Place to put the received data.
|27   |Signal PRU 0
|31   |Receive the data.  14 says to get the data directly from PRU 0.  5
says to start with register `r5`.  `dmemBuf` is where to put the data.
|===
