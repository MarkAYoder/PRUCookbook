...
<<more_delay-test>> is much like our other c code, but on line 9 we declare `my_delay_cycles` and then on line 23 and 25 well call it with an argument
of 1.
[[more_delay-test]]
.test-delay.c
[source,c,linenums]
...
The `<<more_makefile>>` has just a couple of additions.  First
`TARGETasm` is the name of the assembler file.  Generally this is set
outside with `Makefil`e with `export TARGETasm=delay`.  Line 53 has the
bold part shown below, added so the assmbler output is also linked in.
...
Finally lines 63-66 are added to assemble the `TARGETasm` file.
...
|15  |Declaring `cycle` and `stall`.  The compiler will optimize these and just
...
|11-20|`dmemBuf` holds the data to be sent to PRU 1.  Each will be transfered
to its corresponding register by `xout()`.
|23-24|Define the interupts we're using.
|29-30|Clear the interrupts.
|33-38|Initialize dmemBuf with easy to recognize values.
|42   |Wait for PRU 1 to signal.
|47   |`pass:[__]xout()` does a direct transfer to PRU 1. 
Page 92 of 
http://www.ti.com/lit/ug/spruhv7b/spruhv7b.pdf[PRU Optimizing C/C++ Compiler, v2.2, User's Guide]
...
|50   |Clear the interupt so it can go again.
|===
...
|10-17|Place to put the received data.
|28   |Signal PRU 0
|32   |Receive the data. The arguments are the same as `xout()`,  
