...
<<more_cycle_list0>>.
[[more_cycle_list0]]
./tmp/pru0-gen/cycle.lst`
[source,asm,linenums]
[subs=+quotes]
----
include::code/cycle.lst[lines=112,118]
----
Here the `LDI` instruction loads the address `0x22000` into `r0`.  This is the
offset to the `CTRL` registers. 
 Later we see
<<more_cycle_list1>>.
[[more_cycle_list1]]
./tmp/pru0-gen/cycle.lst`
...
----
The first `LDI` takes the contents of `r0` and adds the offset 12 to it and
copies 4 bytes into `r`.  This points to `CYCLE`, so `r1` has the contents of
`CYCLE`.

The second `LDI` does the same, but with offset 26, which points to `STALL`,
...
----
So `cycles` is 3 and `stall` is 5. It must be one cycle to clear the GPIO
and 2 cycles to read the `CYCLE` register and save it to DRAM. It's interesting 
there are 5 `stall` cycles. 
