...
* http://exploringbeaglebone.com/[Exploring BeagleBone by Derek Molly]
...
----
Set the `#define` at <1> to the number of samples in one cycle of the waveform 
and get the `#define` at <2> to which waveform and then run `make`.
...
[[blocks_triangle]]
.Reconstructed Tirnagle Waveform
image:figures/triangle.png[Reconstructed Tirnagle Waveform]
...
You can look in `/tmp/pru0-gen/sine.map` to see how much memory is being used.
