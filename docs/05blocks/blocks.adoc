...
|73-77|On PRU 0 this generates the interupt to send to PRU 1.  I found PRU 1 was 
slow to respond to the interupt, so I added some `__delay_cycles()` to keep the
two in sync.
|78-82|PRU 1 then waits for PRU 0 to signal it.  Bit 31 of `__R31` corresponds
to the Host-1 channel which `configInitc()` set up. We also clear event 16 so 
PRU 0 can set it again.
|===

=== Poling an Input at Regular Intervals.
==== Problem
You have an input pin that need to be read at regular intervals.
