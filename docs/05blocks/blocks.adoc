...
[NOTE]
====
See <<../08ai/ai.html#ai_device_tree, Configuring pins on the AI via device trees>> for configuring 
pins on the AI.
====
...
|4   |This include has addresses for the GPIO ports and some bit positions for some
of the headers.
|===
...
[NOTE]
====
See <<../08ai/ai.html#ai_device_tree, Configuring pins on the AI via device trees>> for all
...
|19  |This is like line 17, but `~gpio` inverts all the bits in `gpio` so that where we had a 1, there is now a 0.  This 0 is then ANDed with `pass:[__]R30` setting the corresponding bit to 0.  Thus we are clearing the bit we selected.
|===
...
I used a 10K&Omega; variable resistor and a 0.022&mu;F capacitor.
Probe the circuit between the resistor and the capacitor and adjust the resistor 
until you get a good looking waveform.
====
...
|28   |Get the j^th^ color and mask off all but the i^th^ bit.  `(0x1<<i)` takes
...
1.  The include on line 10 is switched to `#include "resource_table_1.h"`
(0 is switched to a 1)
2.  Line 17 is switched to `#define HOST_INT			((uint32_t) 1 << 31)`
