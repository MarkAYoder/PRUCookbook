...
which is in the ((heap)).  Finallly `shared_5` is a pointer to where the value
is stored.
...
=== Auto Initialization of Builtin LED Triggers
==== Problem
I see the builtin LEDs blink to their own patterns.  How do I turn this off? 
Can this be automated?

==== Solution
Each builtin LED has a default action (trigger) when the Bone boots up. 
...
when the builtin flash memory is in use.  You can turn this off using:
[subs=+quotes]
----
