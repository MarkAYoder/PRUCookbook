...
|91   |Receive all the data from the ARM, store it in `payload[]`.
|95-103|The data sent is:  index red green blue.  Pull off the index.  If it's 
in the right range, pull off the red, green and blue values.
|105  |The NeoPixels want the data in GRB order.  Shift and OR everything 
together.
|108-131|If the `index` = -1, send the contents of `color` to the LEDs.  This
