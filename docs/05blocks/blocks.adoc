...
[[blocks_neo1]]
.Code to turn all NeoPixels's white
[source,c,linenums]
[subs=+quotes]
----
include::code/neo1.c[]
...
The code in <<blocks_neo1>> define these times in lines 6-9.  The `/5` is because 
each instruction take 5ns.  Lines 24-27 then set the output to 1 for
the desired time and then to 0 and keeps repeating it.  <<blocks_zero_scope>> 
