...
bone$ *ls*
gpio1.pru0.c  gpio_setup.sh  Makefile
----
...
----
bone$ *make*
/var/lib/cloud9/common/Makefile:28: MODEL=TI_AM335x_BeagleBone_Black,TARGET=gpio1.pru0,COMMON=/var/lib/cloud9/common
...
LD	/tmp/cloud9-examples/gpio1.pru0.o
-	copying firmware file /tmp/cloud9-examples/gpio1.pru0.out to /lib/firmware/am335x-pru0-fw
write_init_pins.sh
...
rm /tmp/cloud9-examples/gpio1.pru0.o
----
...
give the filename, without the `.c` extension (`gpio1.pru0`).
The file extension `.pru0` gives the number of the PRU you are using 
...
This assumes `PRUN` is set to the number of the PRU you are using.
...
If fact there are PRU versions of many of the standard code generation tools.
