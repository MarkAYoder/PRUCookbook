...
You can also specify them what running `make`.
[subs=+quotes]
----
...
|19,20|This gives the stack and heap sizes. `STACK_SIZE` is the size of section
`.stack` and `HEAP_SIZE` is the size of the `.bss` section.
|22,23|Flags for the c compiler and the linker
...
.Finding the PRUs
`bone$ *cd /sys/devices/platform/ocp/4a32600*.pruss-soc-bus/4a300000.pruss*` +
`bone$ *ls*` + 
