...
`TARGET` to the file you want to compile. You can override these on the
command line.
...
Here is the stardard Makefile (`Makefile`) used throughout this book.
...
|12,13|These tell where to find the PRU compiler and the support libraries and 
...
|20|This gives the stack and heap sizes. `STACK_SIZE` is the size of section
`.s21tack` and `HEAP_SIZE` is the size of the `.bss` section.
|23,25|Flags for the c compiler and the linker
|27-33|Here we map the PRU number to its physical address. This is needed later
when loading the PRU. These addresses are fixed, no matter which Beagle you
are using.
|35   |This computes the path to the given PRU. If you look in this directory
...
|37   |Since this is the first rule, it's the one that's run what you enter
`make` without a target. So here we stop the PRU, install the code and then
start the PRU.
|39-41|This rule stops the current PRU by writing the command `stop` into the 
...
|47-49|The PRU code is installed by simply copying the generated `*.out` file
to `/lib/firmware/am335x-pru__X__-fw` where __X__ is either 0 or 1 depending 
on the PRU being used.
|51-58|Rules for compiling and linking. Notice the `clpru` command has `-D=PRUN=$(PRUN)`.
