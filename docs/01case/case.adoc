...
There are many http://../projects.html[projects] that use the PRU.

to do things that can't be 
...
<<../06io/io.adoc#io_uio,Configuring for UIO Instead of RemoteProc>> to configure your Bone to use UIO.  You'll have to use and older kernel to do this.
====
[NOTE]
====
This has only been tested on the BeagleBone Black.
====
...
bone$ *cd LEDscape*
----
Now edit `opc-server.c` and comment out line 737.
...
menu and select *MultiSync* as shown in <<case_multisync>>.
[[case_multisync]]
.Using MultiSync page to switch to Bridge mode
image::figures/fpp_multisync.png[MultiSync]

Select *FPP* by checking the box on the right and select *Set to Bridge*
for *Action for selected system*.  Then click *>Run*.  it will take a moment
to restart FPPD.

Then go to the *Status/Control* menu and select *Status Page*.  It is now in 
Bridge mode.
