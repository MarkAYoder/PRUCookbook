...
=== PocketScroller
https://www.hackster.io/daniel-kulp/pocketscroller-led-panel-cape-for-pocketbeagle-fe12a6[hackset.io pocketscroller]
...
<<case_xlights_matrix>>, the click on the black area where you want your 
matrix to appear.
...
First we ned to setup FPP to take input from xLights.  Do this by going to 
...
an effect from the *Effects* box to the time line that below it.  Drop it to 
...
image:figures/xlights_seq_drag.png[Drag an effect to the time line]

The setup requires the host computer to end the annimation data to the Bone. 
...
Once you sequence is uploded, got to *Content Steup* and select *Playlists*. 
