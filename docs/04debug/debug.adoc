...
[WARNING]
====
Unfortunatley https://git.ti.com/cgit/pru-software-support-package/pru-software-support-package/tree/examples/am572x
doesn't have a PRU UART example, therefore there's no example for the PRU UART 
on the AI.
====

The BBAI has a 3 pin JST connector for attaching to the UART as shown in
<<debug_AI_uart>>

[[debug_AI_uart]]
.AI uart connector
image::figures/BB_AI_uart.jpg[AI uart  connector]
https://www.element14.com/community/community/designcenter/single-board-computers/next-genbeaglebone/blog/2019/10/06/beaglebone-ai-bb-ai-getting-started#jive_content_id_Serial_Console_Cable[BeagleBone AI (BB-AI) - Getting Started]
gives details on how to use it from the ARM.
...
----
bone$ *export PRUN=0*
bone$ *export TARGET=uart1.pru0*
bone$ *make*
-    Stopping PRU 0
[sudo] password for debian: 
stop
CC	uart1.pru0.c
"uart1.pru0.c", line 87: warning #112-D: statement is unreachable
"uart1.pru0.c", line 15: warning #552-D: variable "rx" was set but never used
LD	/tmp/pru0-gen/uart1.pru0.obj
-	copying firmware file /tmp/pru0-gen/uart1.pru0.out to /lib/firmware/am335x-pru0-fw
-    Starting PRU 0
start
----
...
.uart1.pru0.c output
image:figures/uart1.pru0.png[uart1.pru0.c output]

Here's the code (`uart1.pru0.c`) that does it.

[[debug_uart1]]
.uart1.pru0.c
[source,c,linenums]
----
include::code/uart1.pru0.c[uart1.pru0.c]
----
