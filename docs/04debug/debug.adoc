...
on your host computer as shown in <<debug_ftdi>>.footnote:[FTDI images are from the
...
<<debug_ftdi_connector>> which marks the ground pin, pin 1.  

[[debug_ftdi_connector]]
.FTDI connector
image::figures/FTDIconnector.jpg[FTDI connector]

The <<debug_FTDI>> table shows which pins connect where and <<debug_ftdi_pins>>
is a wiring diagram for the BeagleBone Black.

[[debug_FTDI]]
.Wriing for FTDI cable to Beagle
[width=35%, cols="1,4,4,4,4,4"]
|===
|FTDI pin|Color |Black pin|AI 1 pin     |AI 2 pin|Function

|0       |black |P9_1     |P8_1         |P8_1     |ground
|4       |orange|P9_24    |P8_43        |P8_33a   |rx
|5       |yellow|P9_26    |P8_44        |P8_31a   |tx
|===

[[debug_ftdi_pins]]
.FTDI to BB Black
image::figures/FTDIhookup_bb.png[FTDI to BB Black]
...
[TIP]
====
On the Black, ither PRU0 and PRU1 can run this code.  Both have access to the same UART.
====

On the Black, for either of these you will need to set the pin muxes.
...
[source,c,linenums]
----
include::code/uart1.pru1_0.c[uart1.pru1_0.c]
----
[NOTE]
====
I'm using the AI version of the code since it uses variables with more desciptive
names.
====
...
.uart2.pru0.c output
image:figures/uart2.pru0.png[uart2.pru0.c output]
...
[source,c,linenums]
----
include::code/uart2.pru0.c[uart2.pru0.c]
----
